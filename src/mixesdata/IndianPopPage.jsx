...
const IndianPopPage = () => {
  ...
  if (IndianPopData.length === 0)
    return <p className="p-4 sm:p-6 text-sm sm:text-base md:text-lg text-center">No video found</p>;

  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 md:gap-6 p-2 sm:p-4 md:p-6">
      {IndianPopData.map((video) => (
        <div
          key={video.id}
